...
long long evaal(const vector<long long>& nums, const vector<string>& op) {
    ...
    for (size_t i = 0; i < op.size(); ++i) {
        if (op[i] == "+") {
            ...
            result = stoll(to_string(result) + to_string(nums[i + 1]));
        }
    }
    return result;
}
...
long long zzzz(const string& input) {
    ...
    while (getline(input, line)) {
        ...
        vector<string> operators(numOperators);
